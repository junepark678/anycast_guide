...
router bgp 65001
  # Define neighbor
  neighbor 192.0.2.254 remote-as 64512
  
  # Description
  neighbor 192.0.2.254 description "Primary Upstream"
  
  # Update source (use specific interface)
  neighbor 192.0.2.254 update-source eth0
  
  # Multihop (for non-directly-connected peers)
  neighbor 192.0.2.254 ebgp-multihop 2
  
  # Timers (keepalive holdtime)
  neighbor 192.0.2.254 timers 30 90
  
  # MD5 authentication
  neighbor 192.0.2.254 password secret123
  ...
  neighbor 192.0.2.254 passive
  
  # BFD integration
  neighbor 192.0.2.254 bfd
```
...
router bgp 65001
  ...
  # Apply to neighbors
  neighbor 192.0.2.254 peer-group upstream
  neighbor 192.0.2.253 peer-group upstream
```
...
router bgp 65001
  ...
  address-family ipv4 unicast
    ...
    neighbor 192.0.2.254 route-map EXPORT-UPSTREAM out
    neighbor 192.0.2.254 route-map IMPORT-UPSTREAM in
    neighbor 192.0.2.254 prefix-list ALLOWED-OUT out
    neighbor 192.0.2.254 prefix-list ALLOWED-IN in
  ...
  address-family ipv6 unicast
    ...
    neighbor 2001:db8::1 route-map EXPORT-V6 out
  exit-address-family
```
...
ip prefix-list ALLOWED-IN seq 10 permit 0.0.0.0/0 le 24  # Allow up to /24
ip prefix-list ALLOWED-IN seq 20 deny any
...
router bgp 65001
  ...
  neighbor 169.254.169.254 description Vultr
  ...
  address-family ipv4 unicast
    ...
    neighbor 169.254.169.254 route-map EXPORT out
  exit-address-family
!
ip prefix-list ANYCAST seq 10 permit 203.0.113.0/24
!
...
ip route 203.0.113.0/24 Null0 250

# Track object for health (requires track daemon)
track 1 ip route 127.0.0.1/32 reachability
!
ip route 203.0.113.0/24 Null0 track 1
```
...
router bgp 65001
  ...
  neighbor 192.0.2.2 description Backup-ISP
  !
  address-family ipv4 unicast
    neighbor 192.0.2.1 route-map PRIMARY-EXPORT out
    neighbor 192.0.2.2 route-map BACKUP-EXPORT out
  exit-address-family
!
...
router bgp 65001
  ...
  !
  neighbor 169.254.169.254 remote-as 64515
  neighbor 169.254.169.254 peer-group UPSTREAM
  neighbor 169.254.169.254 description Vultr-Primary
  neighbor 169.254.169.254 password secretpassword
  !
  address-family ipv4 unicast
    network 203.0.113.0/24
