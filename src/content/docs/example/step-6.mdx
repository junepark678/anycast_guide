...
# Configuration
ANYCAST_IP="2001:db8:abcd::1"
CHECK_DOMAIN="example.com"
EXPECTED_RESULT="2001:db8:abcd::10"
BIRD_PROTOCOL="static_anycast_v6"
...
check_dns() {
    local result
    result=$(dig +short +time=2 +tries=1 @::1 "$CHECK_DOMAIN" AAAA 2>/dev/null || echo "FAILED")
...
def check_dns():
    try:
        result = subprocess.run(
            ['dig', '+short', '@::1', 'example.com', 'AAAA'],
            capture_output=True, text=True, timeout=5
        )
        return '2001:db8:abcd::10' in result.stdout
    except:
        return False

if check_dns():
    print('announce route 2001:db8:abcd::/48 next-hop self')
else:
    print('withdraw route 2001:db8:abcd::/48 next-hop self')
...
healthcheck:
  ...
  checks:
    - type: dns
      host: ::1
      query: example.com
      expected: 2001:db8:abcd::10
  actions:
    healthy: "birdc enable static_anycast_v6"
    unhealthy: "birdc disable static_anycast_v6"
```
  </TabItem>
</Tabs>
...
dig @2001:db8:abcd::1 whoami.example.com TXT
