...
For our example resources (AS212345, 203.0.113.0/24, 2001:db8:abcd::/48):
...
```
  ...
  <TabItem label="route Object (IPv4)">
```
route:          203.0.113.0/24
descr:          Example Anycast IPv4
origin:         AS212345
mnt-by:         EXAMPLE-MNT
created:        2024-01-15T12:00:00Z
last-modified:  2024-01-15T12:00:00Z
...
3. **Verify creation**
   ```bash
   # Check IPv4 route object
   whois -h whois.radb.net 203.0.113.0/24
   
   # Check IPv6 route object
...
2. **Create ROAs for your prefixes**
   ...
   - **Prefix**: The IP range (e.g., 203.0.113.0/24)
   - **Maximum Length**: Usually same as prefix length for anycast
   - **Origin ASN**: Your ASN (AS212345)

3. **Example ROA entries**
   
   | Prefix | Max Length | Origin ASN |
   |--------|------------|------------|
   | 203.0.113.0/24 | /24 | AS212345 |
   | 2001:db8:abcd::/48 | /48 | AS212345 |
...
curl -s "https://rpki-validator.ripe.net/api/v1/validity/AS212345/203.0.113.0/24" | jq

# Using Cloudflare's RPKI toolkit
curl -s "https://rpki.cloudflare.com/api/v1/prefix/203.0.113.0/24" | jq
```
