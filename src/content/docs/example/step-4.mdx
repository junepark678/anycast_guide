...
1. **Enable BGP on your account**
   ...
   Navigate to Account → BGP in the Vultr dashboard and request BGP access.
...
3. **Get BGP session details**
   ...
   Vultr provides:
   - Neighbor IP (typically `169.254.169.254`)
...
define VULTR_NEIGHBOR = 169.254.169.254;

# Vultr IPv4 BGP Session
protocol bgp vultr_v4 from upstream_v4 {
    neighbor VULTR_NEIGHBOR as VULTR_ASN;
    ...
    ipv4 {
        import none;
        export filter {
            if is_my_prefix_v4() then accept;
            reject;
        };
    };
}
...
```bird
# /etc/bird/bird.conf
# Complete Anycast Configuration for AS212345 with Vultr
...
define MY_IPv4 = 203.0.113.0/24;
define MY_IPv6 = 2001:db8:abcd::/48;
define VULTR_ASN = 64515;
define VULTR_NEIGHBOR = 169.254.169.254;

# Functions
function is_my_prefix_v4() {
    if net = MY_IPv4 then return true;
    return false;
}
...
3. **Check BGP session status**
   ```bash
   sudo birdc show protocols all vultr_v4
   ```
   
   Look for `Established` state:
   ```
   vultr_v4   BGP        ---        up     12:34:56    Established
...
4. **Verify route export**
   ...
   ```
   203.0.113.0/24     blackhole [static_anycast_v4 12:00:00] * (200)
   ```
</Steps>
...
# Check route details
sudo birdc show route for 203.0.113.0/24 all
```
...
| Routes not exported | Filter rejecting routes | Check `is_my_prefix_v4()` function |
